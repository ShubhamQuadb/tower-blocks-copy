define(["view/draw", "model/game", "controller/gameLogic", "controller/action", "model/canvas", "model/sounds"], function (Draw, Game, GameLogic, Action, Canvas, Sounds) {
    ...
    var changeTextSize = function changeTextSize() {
        ...
        // DISABLED FOR KAIOS - Don't pause game for small screens (240x320 is intentional)
        // KaiOS devices are fixed at 240x320 which is below the desktop minimum
        Game.screenTooSmall = false;
        
        // Set appropriate font size for KaiOS
        if (width <= 240 && height <= 320) {
            Canvas.context.font = "12px Arial"; // KaiOS optimized
    ...
    var draw = function draw() {
        ...
        if (Game.screen === "game") {
            Draw.drawGame();
            // Only check collisions when game has actually started (not during wave screen)
