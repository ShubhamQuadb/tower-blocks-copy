...
var Block = /** @class */ (function () {
    function Block(block) {
        ...
        // set the dimensions from the target block, or defaults (reduced for KaiOS)
        this.dimension.width = this.targetBlock ? this.targetBlock.dimension.width : 6;
        this.dimension.height = this.targetBlock ? this.targetBlock.dimension.height : 1.5;
        this.dimension.depth = this.targetBlock ? this.targetBlock.dimension.depth : 6;
        this.position.x = this.targetBlock ? this.targetBlock.position.x : 0;
        this.position.y = this.dimension.height * this.index;
        this.position.z = this.targetBlock ? this.targetBlock.position.z : 0;
        this.colorOffset = this.targetBlock ? this.targetBlock.colorOffset : Math.round(Math.random() * 100);
        // set color - Vibrant & Different colors for each block
        if (!this.targetBlock) {
            this.color = 0x667eea; // Purple base
        }
        else {
            var offset = this.index + this.colorOffset;
            // More vibrant and distinct color variations
            var r = Math.sin(0.5 * offset) * 127 + 128; // Full range red (0-255)
            var g = Math.sin(0.5 * offset + 2) * 127 + 128; // Full range green (0-255)
            var b = Math.sin(0.5 * offset + 4) * 127 + 128; // Full range blue (0-255)
...
var Game = /** @class */ (function () {
    function Game() {
        ...
        document.addEventListener('keydown', function (e) {
            // Spacebar (32), Enter (13), Number 5 (53 keyboard, 101 numpad)
            if (e.keyCode == 32 || e.keyCode == 13 || e.keyCode == 53 || e.keyCode == 101)
                _this.onAction();
        });
    ...
    Game.prototype.addBlock = function () {
        ...
        // Hide instructions after first block is placed (length >= 3 means 2 blocks placed)
        if (this.blocks.length >= 3)
            this.instructions.classList.add('hide');
    };
    Game.prototype.endGame = function () {
        var finalScore = this.blocks.length - 2;
        console.log("Tower Blocks Jio Phone: Game Over - Score: " + finalScore);
        
        // Post score to JioGames SDK
        if (typeof postScore === 'function') {
            postScore(finalScore);
            console.log("Tower Blocks Jio Phone: Score posted to SDK - " + finalScore);
        }
        ...
        if (typeof showAd === 'function') {
            setTimeout(function() {
                showAd();
                console.log("Tower Blocks Jio Phone: Showing midroll ad");
            }, 1500);
        }
...
console.log("Tower Blocks Jio Phone: Initializing JioGames SDK...");

// Get user profile on game initialization
if (typeof getUserProfile === 'function') {
    getUserProfile();
    console.log("Tower Blocks Jio Phone: getUserProfile() called");
}
...
setTimeout(function() {
    if (typeof gameCacheAd === 'function') {
        gameCacheAd();
        console.log("Tower Blocks Jio Phone: gameCacheAd() called - Ads caching started");
    }
}, 2000);